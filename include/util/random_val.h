...
inline constexpr __host__ __device__ float PI() { return 3.14159265358979323846f; }
...
template <typename RNG>
inline __host__ __device__ float2 random_val_2d(RNG& rng) {
	return {rng.next_float(), rng.next_float()};
}

inline __host__ __device__ float3 cylindrical_to_dir(const float2& p) {
...
inline __host__ __device__ float2 dir_to_cylindrical(const float3& d) {
	const float cos_theta = fminf(fmaxf(-d.z, -1.0f), 1.0f);
	float phi = atan2(d.y, d.x);
	return {(cos_theta + 1.0f) / 2.0f, (phi / (2.0f * PI())) + 0.5f};
}

inline __host__ __device__ float2 dir_to_spherical(const float3& d) {
	const float cos_theta = fminf(fmaxf(d.z, -1.0f), 1.0f);
...
inline __host__ __device__ float2 dir_to_spherical_unorm(const float3& d) {
	float2 spherical = dir_to_spherical(d);
	return {spherical.x / PI(), (spherical.y / (2.0f * PI()) + 0.5f)};
}

template <typename RNG>
inline __host__ __device__ float3 random_dir(RNG& rng) {
...
__device__ __host__ float3 fibonacci_dir(uint32_t i, const float2& offset) {
	...
	return cylindrical_to_dir(float2{fractf((i+epsilon) / (N_DIRS-1+2*epsilon) + offset.x), fractf(i / GOLDEN_RATIO + offset.y)});
}

template <typename RNG>
inline __host__ __device__ float2 random_uniform_disc(RNG& rng) {
	float2 sample = random_val_2d(rng);
	float r = sqrtf(sample.x);
	float sin_phi, cos_phi;
	sincosf(2.0f * PI() * sample.y, &sin_phi, &cos_phi);
	return float2{ r * sin_phi, r * cos_phi };
}

inline __host__ __device__ float2 square2disk_shirley(const float2& square) {
	float phi, r;
	float a = square.x;
	float b = square.y;
...
inline __host__ __device__ __device__ float3 cosine_hemisphere(const float2& u) {
...
template <typename RNG>
inline __host__ __device__ float3 random_dir_cosine(RNG& rng) {
	return cosine_hemisphere(random_val_2d(rng));
}

template <typename RNG>
inline __host__ __device__ float3 random_val_3d(RNG& rng) {
	return {rng.next_float(), rng.next_float(), rng.next_float()};
}

template <typename RNG>
inline __host__ __device__ float4 random_val_4d(RNG& rng) {
...
inline __host__ __device__ uint2 sobol2d(uint32_t index) {
	return {sobol(index, 0), sobol(index, 1)};
}

inline __host__ __device__ uint4 sobol4d(uint32_t index) {
...
inline __host__ __device__ uint4 shuffled_scrambled_sobol4d(uint32_t index, uint32_t seed) {
	index = nested_uniform_scramble_base2(index, seed);
	auto X = sobol4d(index);
    X.x = nested_uniform_scramble_base2(X.x, hash_combine(seed, 0));
    X.y = nested_uniform_scramble_base2(X.y, hash_combine(seed, 1));
    X.z = nested_uniform_scramble_base2(X.z, hash_combine(seed, 2));
    X.w = nested_uniform_scramble_base2(X.w, hash_combine(seed, 3));
	return X;
}

inline __host__ __device__ uint2 shuffled_scrambled_sobol2d(uint32_t index, uint32_t seed) {
	index = nested_uniform_scramble_base2(index, seed);
...
inline __host__ __device__ float4 ld_random_val_4d(uint32_t index, uint32_t seed) {
	constexpr float S = float(1.0/(1ull<<32));
	uint4 x = shuffled_scrambled_sobol4d(index, seed);
	return {(float)x.x * S, (float)x.y * S, (float)x.z * S, (float)x.w * S};
}

inline __host__ __device__ float2 ld_random_val_2d(uint32_t index, uint32_t seed) {
	constexpr float S = float(1.0/(1ull<<32));
	uint2 x = shuffled_scrambled_sobol2d(index, seed);
...
inline __host__ __device__ float2 halton23(size_t idx) {
	return {halton<2>(idx), halton<3>(idx)};
}

// Halton
// inline __host__ __device__ float2 ld_random_pixel_offset(const uint32_t spp) {
// 	float2 offset = float2(0.5f) - halton23(0) + halton23(spp);
...
// Scrambled Sobol
inline __host__ __device__ float2 ld_random_pixel_offset(const uint32_t spp, const uint32_t seed = 0xdeadbeef) {
	float2 offset = ld_random_val_2d(spp, seed);
	offset.x = fractf(offset.x);
	offset.y = fractf(offset.y);
	return offset;
}

inline __host__ __device__ float ld_random_t_offset(const uint32_t spp, const uint32_t seed = 0xdeadbeef) {
	float offset = ld_random_val(spp, seed);
	offset = fractf(offset);
	return offset;
}